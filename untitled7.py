...
activation=M.addVars(20, vtype=gb.GRB.BINARY)
cout_variable_biomasse=M.addVars(20, vtype=gb.GRB.CONTINUOUS)
...
for i in range (20):
 ...
 #   ctrs['apro_vert', i] = M.addConstr(mass['vert', i] <=dispo_avant2025['vert'] , name='appro_vert_max')
   ...
   # ctrs["Ctrs prix ter",i]=M.addConstr(cout_variable_biomasse[i]>=prix_under*gb.quicksum(mass[c,i]*PCI[c] for c in combustible))
    ...
    ctrs["Ctrs vert 2",i]=M.addConstr(gb.quicksum(lambda_rv[k,i] for k in range(9))<=0.96 )

    
    cout_vert[i]=gb.quicksum(prix_total_rv[k]*lambda_rv[k,i] for k in range(9))
    profit[i] = gb.quicksum(unit_profit(c,i) * mass[c,i] for c in combustible)-cout_vert[i] #quota[i]-cout_variable_biomasse[i]
